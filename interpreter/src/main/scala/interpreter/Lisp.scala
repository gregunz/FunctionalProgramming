...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'case :: expr_struct :: ('else :: exprElse :: Nil) :: Nil =>
      eval1(exprElse, env)

    case 'case :: expr_struct :: (v :: e :: Nil) :: ('else :: exprElse :: Nil) :: Nil =>
      val condition = eval1(expr_struct, env)
      if(condition == v)
        eval1(e, env)
      else
        eval1(exprElse, env)
    //eval1('if :: ('= :: expr_struct :: v :: Nil) :: e :: exprElse :: Nil, env)

    case 'case :: expr_struct :: (v :: e :: Nil) :: valuesToExpr =>
      val condition = eval1(expr_struct, env)
      if(condition == v)
        eval1(e, env)
      else
        eval1('case :: expr_struct :: valuesToExpr, env)
      /*val evalIf = 'if :: ('= :: expr_struct :: v :: Nil) :: e :: Nil
      val nextCase = List('case :: expr_struct :: valuesToExpr)
      eval1( evalIf :: nextCase :: Nil, env)*/


    case 'def :: (name :: args) :: body :: rest :: Nil =>
      //val (name :: args) = nameAndArgs
      eval1('def :: name :: ('lambda :: args :: body :: Nil) :: rest :: Nil, env)
