...
object ConcertsPlanner {
   ...
   */
  def isUsedOnce(us: List[Unit], s: Set[Unit]): Boolean = {
    us match {
      case Nil => true
      case u :: us => {
        val bool = s.contains(u)
        bool && isUsedOnce(us, s + u)
      }
    }
    true
  }
  /*
var setOfBands: Set[Band] = Set()
varsMatrix.map{
  case ((band, slot), formula) => {
    val bool = setOfBands.contains(band)
    setOfBands += band
    formula && !bool
  }
}
...
*/
  ...
  def plan(preferences: Map[Band, List[Slot]]): Option[Map[Band, Slot]] = {
    ...
    val desirableSlots: Seq[Formula] = {
      /*or(varsMatrix.map {
        case ((b, s), f) if preferences(b).contains(s) => f
      }.toSeq:_*)
      */

      bands.map( b =>
        or(slots.filter(
          s => preferences(b).contains(s)
        ).map{
          s => varsMatrix((b,s))
        }:_*)
      )
  ...
  def getUniqueSlots(preferences: Map[Band, List[Slot]]): Set[Slot] = {
    preferences.values.flatten
      .foldLeft(Map(): Map[Slot, Int]) {
        (m: Map[Slot, Int], s: Slot) =>
          m.updated(s, m.withDefaultValue(0)(s) + 1)
      }
      .filter { case (s, i) => i == 1 }
      .keySet
  }
